# 现在可交给新窗口的提示词：独立 PM 集成纠偏治理

> 这是非盲的独立治理审查窗口。不是 Resident，不执行实验。治理 PR 发布后，先使用本提示词；operator preflight 必须等本任务真正完成并写回后才能启动。
...
7. 若接受，记录你的独立审查结论并按仓库权限/保护规则用 gh 合并**治理 PR**（不是 #117/#121 等证据 PR）。不要绕过保护规则/required checks；没有权限则明确等待维护者。
