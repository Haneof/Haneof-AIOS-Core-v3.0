...
(USER, SENSOR, PLATFORM, SAFETY) and preventing self-closing cycles such as
Claim -> Summary -> Claim.
"""
...
class CognitionEvidencePolicy:
    """Unified policy layer resolving evidence support closure for cognition.

    Enforces reality leaf closure across ordinary turn, Periodic Review,
    and Cognitive Derivation. Forbids self-confirmation loops (Claim -> Summary -> Claim)
    while allowing legitimate Reality Observation -> Claim -> Revision paths.
    """
    ...
    def _payload_source_refs(
        ...
        *,
        ...
        if not isinstance(raw_refs, list):
            CognitionEvidencePolicy._record_unresolved(
                state,
                owner_ref,
                "invalid_source_refs",
            )
    ...
    def derive_lineage_for_refs(
        ...
        *,
        ...
        grounding_leaf_refs records reality/case leaves reached without using
        an intervening AI semantic assertion (for example, an old Claim) as the only bridge.
        """
        state = _TraversalState(
            leaves={},
            grounding_leaves={},
    ...
    def validate(
        ...
        refs: Sequence[ObjectRef],
        *,
        operation: str = "cognition_evidence",
    ) -> DerivedLineageView:
        """Validate that candidate evidence refs satisfy strict reality leaf closure.

        Rejects ungrounded or circular evidence, such as Claim -> Summary -> Claim loops.
        Accepts evidence whose lineage reaches qualifying reality leaves (USER, SENSOR,
        PLATFORM, SAFETY).
        """
        if not refs:
            raise ValueError(
                f"Cognition evidence policy (leaf-grounded evidence closure rejected) {operation}: "
                "no evidence refs provided; cognition must be grounded in reality evidence"
            )
        lineage = self.derive_lineage_for_refs(tuple(refs))
        if (
            ...
            issues_str = ",".join(lineage.issues) if lineage.issues else "none"
            raise ValueError(
                "Cognition evidence policy (leaf-grounded evidence closure rejected) "
...
# Alias for explicit resolver terminology
EvidencePolicyResolver = CognitionEvidencePolicy