...
| R5 FIX-001 historical temporal cut | **PASS / ALREADY_SAFE** | Fresh headless retained Wake restart test `test_headless_restart_keeps_fix001_historical_knowledge_cut` passed. Independently inspected exact-head Periodic Review test `test_cg001_resumed_periodic_review_uses_original_start_as_read_cut`: a T1 running review resumes at T2 after a late T2 fact+cognition is added, but cockpit/search exclude the T2-only material and resulting writeback retains `learned_at = T1`. |
...
Accepted historical cutoff behavior remains intact. Candidate recovery code adds no replay path that substitutes current World knowledge for a historical execution. Fresh headless retained Wake restart cutoff test passed. In addition, exact-head Periodic Review regression `test_cg001_resumed_periodic_review_uses_original_start_as_read_cut` was independently inspected: it resumes a T1 review at T2 after adding T2-only fact/cognition, proves both cockpit/search exclude that later knowledge, and proves the recovered writeback keeps `learned_at = T1`. The exact-head P15 Periodic Review job `107630397575` completed SUCCESS.
