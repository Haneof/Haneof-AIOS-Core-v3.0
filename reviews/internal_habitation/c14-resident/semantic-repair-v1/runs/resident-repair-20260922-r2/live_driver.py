...
RUN_ID = "c14-sem-repair-resident-20260922-sol-r2"
SESSION_ID = "resident-sem-repair-sol-20260922-r2"
SUBJECT_ID = "user_1"
STARTING_MAIN = "7611fa5059f5dc8a20835cab5b312be2f43d11e8"
BRANCH = "c14/semantic-repair-resident-20260922-sol"
...
def load_state() -> dict[str, Any]:
    state = load_json(
        RUN_STATE,
        {
            ...
            "summary_requests": [],
            "errors": [{"kind": "infrastructure_retry", "attempt": "r1", "reason": "cursor-1 evidence push rejected non-fast-forward after control transport changed the same branch; no cursor 2 was revealed; r2 restarts from a fresh private World"}],
            "silence_count": 0,
            "response_count": 0,
...
class GithubBridge:
    def __init__(self, state: dict[str, Any], store: SQLiteWorldStore):
        ...
        self.pr_number = self._wait_for_pr()
    ...
    def _wait_json_marker(self, marker: str, item_id: str) -> tuple[dict[str, Any], str]:
        prefix = f"AIOS_RESIDENT_{marker} {RUN_ID} {item_id}"
        while True:
    ...
    def wait_event_seen(self, event: Mapping[str, Any]) -> None:
        ...
        prefix = f"AIOS_RESIDENT_EVENT_SEEN {RUN_ID} {event_id}"
        while True:
            for comment in reversed(self._comments()):
                if str(comment.get("body") or "").strip().startswith(prefix):
                    return
            time.sleep(POLL_SECONDS)
