...
RUN_ID = "c14-sem-repair-resident-20260922-sol-r8-3e8a71"
...
COMMENT_LIMIT = 56000
POLL_SECONDS = 4.0
CONTROL_NONCE = "ctl-3e8a71-a9c4f26d"
...
class GithubBridge:
    def __init__(self, state: dict[str, Any], store: SQLiteWorldStore):
        ...
        self.pr_number = int(os.environ.get("RESIDENT_PR_NUMBER", "0"))
        if self.pr_number < 1:
            raise RuntimeError("RESIDENT_PR_NUMBER is required")
    ...
    def post_chunks(self, kind: str, item_id: str, payload: Any) -> None:
        raw = canonical_json(payload, pretty=False)
        chunks = [raw[i:i + COMMENT_LIMIT] for i in range(0, len(raw), COMMENT_LIMIT)] or [""]
        total = len(chunks)
        for i, chunk in enumerate(chunks, start=1):
            body = (
                f"AIOS_RESIDENT_{kind} {RUN_ID} {item_id} PART {i}/{total}\n"
                f"\`\`\`json\n{chunk}\n\`\`\`"
            )
            self._api("POST", f"/issues/{self.pr_number}/comments", {"body": body})
    ...
    def _wait_json_marker(self, marker: str, item_id: str) -> tuple[dict[str, Any], str]:
        prefix = f"AIOS_RESIDENT_CONTROL {CONTROL_NONCE} {RUN_ID} {marker} {item_id}"
        while True:
            for comment in reversed(self._comments()):
                body = str(comment.get("body") or "")
                author = str((comment.get("user") or {}).get("login") or "")
                if author != "Haneof" or not body.startswith(prefix):
                    continue
                payload_text = body[len(prefix):].strip()
                ...
                if not isinstance(payload, dict):
                    raise RuntimeError("control payload must be a JSON object")
                if payload.get("run_id") != RUN_ID:
                    raise RuntimeError(f"control run_id mismatch for {item_id}")
                if marker == "DIRECTIVE" and payload.get("checkpoint_id") != item_id:
                    raise RuntimeError(f"directive checkpoint mismatch for {item_id}")
                if marker == "SUMMARY_RESPONSE" and payload.get("request_id") != item_id:
                    raise RuntimeError(f"summary request mismatch for {item_id}")
    ...
    def wait_event_seen(self, event: Mapping[str, Any]) -> None:
        ...
        prefix = f"AIOS_RESIDENT_CONTROL {CONTROL_NONCE} {RUN_ID} EVENT_SEEN {event_id}"
        while True:
            for comment in reversed(self._comments()):
                body = str(comment.get("body") or "").strip()
                author = str((comment.get("user") or {}).get("login") or "")
                if author == "Haneof" and body.startswith(prefix):
                    return
            time.sleep(POLL_SECONDS)
...
def main() -> int:
    ...
    bridge.post_text("HEARTBEAT", "bridge_ready", "bridge initialized; control accepts only nonce-scoped comments")
    ...
    bridge.post_text("HEARTBEAT", "runtime_ready", "FusedTurnRuntime initialized; no event revealed yet")
    ...
    run_manifest = {
        ...
        "provider_request_attestation": None,
        "control_transport": "nonce-scoped PR comments; only Haneof + exact control prefix accepted",
