...
RUN_ID = "c14-sem-repair-resident-20260922-sol-r7-d42f18"
SESSION_ID = "resident-sem-repair-sol-20260922-r7-d42f18"
SUBJECT_ID = "user_1"
...
COMMENT_LIMIT = 56000
POLL_SECONDS = 4.0
CONTROL_NONCE = "ctl-d42f18-72b5c1"
...
class GithubBridge:
    ...
    def _contents_get(self, relpath: str) -> dict[str, Any] | None:
        ...
        if not isinstance(item, Mapping):
            raise RuntimeError("contents lookup returned non-object")
        return dict(item)

    def _read_branch_json(self, relpath: str) -> dict[str, Any] | None:
        item = self._contents_get(relpath)
        if item is None:
            return None
...
def main() -> int:
    ...
    run_manifest = {
        ...
        "provider_request_attestation": None,
        "control_transport": "PR comments output-only; nonce-scoped branch control JSON input; PR number injected by workflow",
