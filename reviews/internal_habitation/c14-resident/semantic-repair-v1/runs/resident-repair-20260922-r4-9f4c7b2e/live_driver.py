...
RUN_ID = "c14-sem-repair-resident-20260922-sol-r4-9f4c7b2e"
SESSION_ID = "resident-sem-repair-sol-20260922-r4-9f4c7b2e"
SUBJECT_ID = "user_1"
STARTING_MAIN = "7611fa5059f5dc8a20835cab5b312be2f43d11e8"
BRANCH = "c14/resident-semrepair-sol-9f4c7b2e"
...
CONTROL_REPO_PATH = "reviews/internal_habitation/c14-resident/semantic-repair-v1/runs/resident-repair-20260922-r4-9f4c7b2e/resident_control.json"
CONTROL_NONCE = "9f4c7b2e-6a31-4c97-bd28-1f0b88a9d453"
...
class GithubBridge:
    def __init__(self, state: dict[str, Any], store: SQLiteWorldStore):
        ...
        self.control_path = CONTROL_REPO_PATH
        self.control_nonce = CONTROL_NONCE
    ...
    def _read_control(self) -> dict[str, Any] | None:
        encoded_path = urllib.parse.quote(self.control_path, safe="/")
        try:
            payload = self._api("GET", f"/contents/{encoded_path}?ref={urllib.parse.quote(BRANCH, safe='')}")
        except RuntimeError:
            return None
    ...
    def _wait_control(self, kind: str, item_id: str) -> tuple[dict[str, Any], str]:
        while True:
            value = self._read_control()
            if (
                isinstance(value, dict)
                and value.get("run_id") == RUN_ID
                and value.get("nonce") == self.control_nonce
                and value.get("kind") == kind
                and value.get("item_id") == item_id
            ):
                payload = value.get("payload")
                if not isinstance(payload, dict):
                    raise RuntimeError("resident control payload must be an object")
                return payload, "single-window-resident-control"
            time.sleep(POLL_SECONDS)

    def wait_event_seen(self, event: Mapping[str, Any]) -> None:
        event_id = str(event["event_id"])
        git_checkpoint(f"evidence(c14): expose current event {event_id}")
        self._wait_control("EVENT_SEEN", event_id)
    ...
    def model_handler(self, snapshot) -> ModelDirective:
        ...
        git_checkpoint(f"evidence(c14): expose runtime snapshot {cp_id}")

        directive_raw, author = self._wait_control("DIRECTIVE", cp_id)
    ...
    def _summary_request(self, kind: str, request: Any) -> str:
        ...
        git_checkpoint(f"evidence(c14): expose summary request {req_id}")
        raw, author = self._wait_control("SUMMARY_RESPONSE", req_id)
...
def git_checkpoint(message: str) -> None:
    ...
    subprocess.run(["git", "pull", "--rebase", "origin", head_ref], cwd=REPO_ROOT, check=True)
