...
from habitation.io import load_resident_fixture
        ...
        for i, item in enumerate(raw.get("calls") or []):
            calls.append(
                CapabilityCall(
                    ...
                    call_id=str(item.get("call_id") or f"seg002-{i+1}"),
                )
            )
...
class ManualResident:
    ...
    def __call__(self, snapshot: RuntimeSnapshot) -> ModelDirective:
        ...
        self.pending += 1
        envelope = {
            "schema": "aios.segment002.manual-resident.pending.v1",
            "decision_key": key,
...
class ManualSummary:
    ...
    def __call__(self, request: Any) -> str:
        ...
        self.pending += 1
        envelope = {
            "schema": "aios.segment002.manual-summary.pending.v1",
            "decision_key": key,
            "resident_model": "GPT-5.6 Sol (interactive ChatGPT resident)",
            "simulated_clock": None
            if self.target is None or self.target.clock is None
...
def main() -> int:
    ...
    loaded = load_resident_fixture(Path(args.fixture_root), args.manifest)
    if loaded.scenario.subject_id != args.expected_subject:
    ...
    visible = [e for e in loaded.scenario.events if e.deliver_to_resident]
    ids = [e.event_id for e in visible]
    if args.after_event_id not in ids:
    ...
    try:
        ...
        write_state("pending")
        print(f"SEG002_STATUS=pending decision_key={exc}")
        print(f"SEG002_CURSOR={last}")
        print(f"SEG002_WORLD_REVISION={int(target.store.current_world_revision())}")
        print(f"SEG002_RESIDENT_REPLAYED={resident.replayed}")
        print(f"SEG002_SUMMARY_REPLAYED={summary.replayed}")
        return 42
