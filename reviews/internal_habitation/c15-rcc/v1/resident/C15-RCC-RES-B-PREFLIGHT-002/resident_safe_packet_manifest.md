...
| `/work/world.sqlite.writer.lock` | Writer lease | same |
...
- **ProductionResidentHandler** (frozen production path, Path A — external trusted model broker):
  ...
  - `provider_client.invoke(request)` runs **outside the jail** (operator / trusted broker side, with `CLONE_NEWNET` still sealed inside).
  - The **only** production transport is `ExternalBrokerClient` (pinned adapter `bridged_model_handler:ExternalBrokerClient`, HTTPS-only). It is pure transport: HTTP POST of `build_model_request(...)`, no semantic default, no keyword→directive mapping, no timeout fallback answer.
  - `FakeProviderClient` **does not** walk the production path and is **not** a drop-in replacement: its `__init__` raises `RuntimeError` unconditionally, so it can never be instantiated from the production entrypoint. It exists only so a test can assert that the production entrypoint refuses it.
  - There is **no** `RealProviderClient` in the frozen harness. Real B supplies credentials/endpoint via `AIOS_REAL_PROVIDER_API_KEY` + `AIOS_REAL_PROVIDER_ENDPOINT` and the same unchanged `ExternalBrokerClient`; nothing in the file is swapped at release time.
  - Provider identity (`provider` / `model` / `request_id`) is taken from the provider response, or is the literal `UNKNOWN` when the response does not carry it. It is never derived from config strings or self-descriptions.
