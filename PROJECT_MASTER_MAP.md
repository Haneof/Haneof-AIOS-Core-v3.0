...
- Active RC blockers now: CG-001 runtime temporal read cut；CG-003 user-turn IN_DOUBT recovery。CG-002 is DONE.
- S2 当前：`CORE-GAP-FIX-002 = DONE`（#154 ACCEPTANCE_PASS → #143 merge `3d980fad...`）；`CORE-GAP-FIX-001 = GATE / ACCEPTANCE_FAIL`（#161 对 #145 exact `b5a50435...` 发现唯一 blocker：C14 runtime-derived lineage 未遵守 historical Tcut）；`CORE-GAP-FIX-001-CORRECTIVE-001 = GATE / REVIEW_READY`（#145 @ `a56f113a...`，等待新的独立验收）；`CORE-GAP-FIX-003 = GATE / ACCEPTANCE_FAIL`（#162 对 #157 exact `81d62682...` 发现唯一 blocker：initial user-turn claim 与 initial provider-attempt admission 非原子导致可恢复性死区）；`CORE-GAP-FIX-003-CORRECTIVE-001 = GATE / REVIEW_READY`（#157 @ `ac8d5a43...`，等待新的独立验收）。FIX-001 与 FIX-003 可按符号级边界并行开发、串行集成。裁决：`governance/AIOS_CORE_S2_SERIALIZATION_RULING_2026-09-24.md`。`CORE-CI-FIX-001 = DONE`：#150 的首轮 ACCEPTANCE_FAIL 历史保留；#156 corrective ACCEPTANCE_PASS；#146 exact `1eb24e10...` merged as `c3ec4221...`；PR #144 = SUPERSEDED / NOT INTEGRATED。
