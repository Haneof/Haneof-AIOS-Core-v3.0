...
mkdir -p "$RUN"/{release_receipts,cursor_lifecycle,checkpoints}
ING="$RUN/release_receipts/cursor-004-ingest.json"; ACK="$RUN/release_receipts/cursor-004-ack.json"
...
python reviews/internal_habitation/c15-rcc/v1/release/release_operator.py ack --phase A --state "$STATE" --world-db "$WORLD" --sequence 4 --event-id c15rcc-004 --ingest-ref "$REF" | tee "$ACK"
cat > "$RUN/cursor_lifecycle/cursor-004.json" <<JSON
{"cursor":4,"event_id":"c15rcc-004","ingest_ref":"$REF","acked":true}
JSON
python - <<'PY'
...
cp=run/"checkpoints"/"cursor-004-pending-decision.json"
...
def dump(o):
    ...
    return str(o)
def save(kind,obj):
    cp.write_text(json.dumps({"cursor":4,"timestamp":event["occurred_at"],"kind":kind,"payload":dump(obj)},ensure_ascii=False,indent=2,default=str)); raise NeedDecision(kind)
